...
import { FaMusic, FaMobileAlt, FaAdjust, FaSave, FaSyncAlt } from "react-icons/fa";
import { MdChevronLeft } from "react-icons/md";
import logo from "../assets/img/Toodler.png";

const Settings = ({ isMuted, setIsMuted }) => {
    const handleMusicToggle = () => setIsMuted(!isMuted);
    ...
    return (
        <div className="bg-white min-h-screen max-w-md flex justify-center items-start pt-16 relative">
            ...
            <div className="flex flex-col items-center w-[350px]">
                <img src={logo} alt="Toddler logo" className="w-30 h-30 mb-1" width="60" height="60" />
                <h1 className="text-black text-5xl mt-2 mb-15">Settings</h1>

                <div className="bg-blue-300 bg-opacity-50 rounded-xl w-full p-6 flex flex-col gap-6">
                    {/* Music Toggle */}
                    <label className="flex justify-between items-center text-white text-[30px] w-full">
                        <span className="flex items-center"><FaMusic className="mr-2" /> Music</span>
                        <input
                            type="checkbox"
                            checked={!isMuted}
                    ...
                    {/* Other toggles */}
                    <label className="flex justify-between items-center text-white text-[30px] w-full">
                        <span className="flex items-center"><FaMobileAlt className="mr-2" /> Vibration</span>
                        <input type="checkbox" className="toggle-checkbox w-12 h-6 rounded-full bg-blue-700 relative appearance-none cursor-pointer" />
                    </label>

                    <label className="flex justify-between items-center text-white text-[30px] w-full">
                        <span className="flex items-center"><FaAdjust className="mr-2" /> Dark Mode</span>
                        <input type="checkbox" className="toggle-checkbox w-12 h-6 rounded-full bg-blue-700 relative appearance-none cursor-pointer" />
                    </label>

                    <button className="mt-4 bg-blue-400 bg-opacity-50 text-white text-[20px] rounded-md py-2 px-4 flex justify-center items-center gap-2 cursor-not-allowed select-none w-full">
                        Save Progress <FaSave />
                    </button>

                    <button className="mt-2 bg-blue-400 bg-opacity-50 text-white text-[20px] rounded-md py-2 px-4 flex justify-center items-center gap-2 cursor-not-allowed select-none w-full">
                        Reset Progress <FaSyncAlt />
                    </button>
                </div>
            </div>
